...
if(year %in% c(2021:2022)){
  ...
  jan <- files[grepl("exactEarth_202101", files)][1:11]
}
if(year %in% 2015:2020){
  dir <- "D:/AlaskaConservation_AIS_20210225/Data_Raw/2015/"
  files <- paste0(dir, list.files(dir, pattern='.csv'))
  jan <- files[grepl("2015-01", files)][1:2]
}
...
}else(MoName <- sub("-", "", substr(csvList[[1]][1], nchar(csvList) - 13, nchar(csvList) - 7)))
...
# AISspeed1$newseg <- ifelse(AISspeed1$timediff > 24, 1, 
#                            ifelse(AISspeed1$distdiff > 200, 1, 0))
